...
function download()
{
	...
	if (in_array($last_filename, $file_list)) {
		...
		if (!empty($file_list)) {
			printf("WARN: last file(%s) not found, downloading all videos\n", $last_filename);
		}
	}
...
{
	if (is_file('last_downloaded_video')) {
		return trim(file_get_contents('last_downloaded_video'));
	}
}
function set_last_filename($filename)
{
	file_put_contents('last_downloaded_video', $filename);
}
function get_file_list()
{
	global $sd_host, $status;
	$list_html = @file_get_contents(sprintf('http://%s/photo?fdir=100CANON&vtype=0&ftype=1',$sd_host));
	if (empty($list_html)) {
		if ($status) {
	...
	preg_match_all('#download\?fname=([^&]*)&fdir=[^&]*&folderFlag=0#ms', $list_html, $matches);
	if (empty($matches[1])) {
	...
	return array_reverse(array_unique($matches[1]));
}
